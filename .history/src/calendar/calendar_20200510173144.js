...
export class Calendar  {
    ...
    build(x) {
        ...
        this.calfrag = $(new DocumentFragment());
        this.calfrag.append(DoubleCal);
        this.calfrag.find(".my1 span:nth-of-type(1)").first().empty().append(this.getM(this.sMonth));
        this.calfrag.find(".my1 span:nth-of-type(2)").first().empty().append(this.getY(this.sMonth,this.sYear));
        this.calfrag.find(".my2 span:nth-of-type(1)").first().empty().append(this.getM(this.sMonth+1));
        this.calfrag.find(".my2 span:nth-of-type(2)").first().empty().append(this.getY(this.sMonth+1,this.sYear));
        this.fragment.append(this.calfrag);

        this.prev = $(`<li class="prev">&#10094;</li>`);
        this.next = $(`<li class="next">&#10095;</li>`);
        this.prev.on("click", () => { this.reload(0) });
        this.next.on("click", () => { this.reload(1) });
        const fragment.find(".first").prepend(this.prev);
        const fragment.find(".second").prepend(this.next);
        
        [...Array(this.getFirstDay(2020,parseInt(this.sMonth))).keys()].forEach( () => {
          const fragment.find(".set-one").append(`<li><span class="leer"></span></li>`); 
        });
        
        [...Array(this.getAllDays(2020,parseInt(this.sMonth))).keys()].forEach(element => {
          const fragment.find(".set-one").append(`<li>${element+1}</li>`); 
        });

        [...Array(this.getFirstDay(2020,parseInt(this.sMonth)+1)).keys()].forEach( () => {
          const fragment.find(".set-two").append(`<li><span class="leer"></span></li>`); 
            });
        
        [...Array(this.getAllDays(2020,parseInt(this.sMonth)+1)).keys()].forEach(element => {
          const fragment.find(".set-two").append(`<li>${element+1}</li>`); 
        });
