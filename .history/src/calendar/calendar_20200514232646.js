...
export class Calendar  {
    constructor(){
        ...
        this.endPin = (x,y) => {
          ...
          while (i <= x) {
            console.log(i);
            const fr = ".set-one li:nth-child("+i+")";
            //$(fr).removeClass("reserve");remove all above start
            $(fr).addClass("reserve");
            i++;
          }
        }
        this.startPin = (x) => {
          ...
          let from = parseInt($(x).text());
          //from = parseInt(from)+2;
          // const fr = ".set-one li:nth-child("+from+")";
          console.log(from,"-",lis.length);
          //for () {};
          let i = from + 1;
          console.log(i,"-",lis.length);
          while (i <= lis.length) {
              //console.log(i);
              const fr = ".set-one li:nth-child("+i+")";
              $(fr).on("mouseenter", () => { 
                console.log($(fr).text(),from) 
                this.endPin(parseInt($(fr).text()),from);
              })
              //$(fr).on("mouseenter", () => { $(fr).toggleClass("reserve") })
              i++;
          }
          // $(fr).on("mouseenter", () => { $(fr).toggleClass("reserve") })
        }
