  ...
  if(text === "Koszyk") {
    ...
    anchor.on('mouseenter', () => {  
      ...
      if(!$.isEmptyObject(myOrdFin)){
        const roomb = !!myOrdFin.room ? "Pokój na termin <br>"+myOrdFin.start.day+" "+myOrdFin.start.month+"-"+myOrdFin.end.day+" "+myOrdFin.end.month+"<br>oraz <br>":"";//location.pathname = "/";
        
        const treats = "";
        if()myOrdFin.treatments.length>0 ? "zabiegi w ilości: "+myOrdFin.treatments.length:"";
