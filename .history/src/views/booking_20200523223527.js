  ...
  //return servePaths.reDirect("rooms").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      servePaths.reDirect("rooms").then(data => {
        ...
        return outroom = String(bookedRoom.name)
        
      });
      
      fragment.find(".addhere").append(`<div class="row mx-5 alert d-flex justify-content-between alert myro alert-secondary">zupa${outroom}
      </div>`)
