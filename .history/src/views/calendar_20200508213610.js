...
export class Calendar  {
    ...
    build(x) {
        ...
        this.nextMonth = this.months[parseInt(this.Month) + 1 > 11 ? (parseInt(this.Month) + 1) - 12 : parseInt(this.Month) + 1];
        this.getAllDays = (y,m) => {
            return new Date(y, (m + 1), 0).getDate();
        };
        this.getFirstDay = (y,m) => {
        ...
        [...Array(this.getFirstDay(2020,parseInt(this.Month))).keys()].forEach( () => {
            this.fragment.find(".set-one").append(`<li><span class="leer"></span></li>`); 
        });
        
        [...Array(this.getAllDays(2020,parseInt(this.Month))).keys()].forEach(element => {
            this.fragment.find(".set-one").append(`<li>${element+1}</li>`); 
        });

        [...Array(this.getFirstDay(2020,parseInt(this.Month)+1)).keys()].forEach( () => {
