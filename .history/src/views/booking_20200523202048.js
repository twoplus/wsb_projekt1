...
export const booking = () => {
  ...
  //return servePaths.reDirect("rooms").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      servePaths.reDirect("rooms").then( roomData => {
        readData = roomData.forEach( item => {
          return readData.push([string(item.id,item.name,item.price])
        })
        
      });
      // for (item of readData) {
      //   console.log(item.id, myOrderFin.room)
      //   out = item.id == myOrderFin.room ? item : false;
      // }
      //readData = [...readData]
      console.log(">>>",readData.length,readData,readData[1])
