...
export const booking = () => {
  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    if(items.length != 0){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row m-5"><div class="btn btn-start offset-4 col-4 btn-primary">Zarezerwuj pobyt</div></div><div id="tst"></div>`);
    ...
    fragment.find("#tst").append(`<div class="row alert alert-dark">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}</div>`)
    fragment.find("#tst").append(`<div class="row alert alert-dark">${myOrderFin.treatments} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}</div>`)
