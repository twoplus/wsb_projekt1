...
export const booking = () => {
  ...
  return servePaths.reDirect("rooms").then(data => { 
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      let total = parseInt(bookedRoom.price);
      ...
      <a class="btn btn-danger btn-del "  role="button"><i class="fa fa-trash" aria-hidden="true"></i></a>
      </div>`)
      
      myOrderFin.treatments.forEach(
        (treat) => {
          fragment.find(".addhere").append(`<div class="row mx-5  d-flex justify-content-between alert alert-secondary">${treat[1]} cena: ${treat[2]}zł
      ...
      <a class="btn btn-success btn-add" href="#" role="button">Przejdź do kasy</a>
      </div>`)

      fragment.find(".btn-del").on("click",() => {
        console.log("kasuje")
      })
    }
