...
export const treatments = () => {
  ...
  return servePaths.reDirect("treatments").then(items => {
    ...
    <p class="text-center s-count text-success">Wybranych: <span style="font-size: 30px">${!!myOrder.treatments?myOrder.treatments.length:0}</span></p>
    ...
    fragment.find(".btn-bt").on("click", (event) => {
      ...
      if(!!myOrder) {
        ...
        $(".s-count").text("Wybranych: <span>"+myOrder.treatments.length+"</span");
      } else {
        location.pathname = "/";
      }
    })
