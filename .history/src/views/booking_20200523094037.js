...
export const booking = () => {
  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    if(items.length != 0){//items.length != 0
      fragment.find(".addhere").append(`<br><br><p class="display-2 text-center">Twój koszyk jest pusty</p>`);
    ...
    fragment.find("#tst").append(`<div class="row m-5 alert alert-dark">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}</div>`)
    fragment.find("#tst").append(`<div class="row m-5 alert alert-dark">${myOrderFin.treatments} </div>`)
