  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    <div class="row mr-1 mb-4">
      <div class="media col-12  m-1">
        <div class="media-body  m-1 ">
          <p class="lead m-1">Dodaj zabieg do zamówienia<a class="btn btn-primary btn- float-right" href="#" role="button"><i class="fa fa-plus" aria-hidden="true"></i></a></p>
        </div>
      </div>
    </div>
    `);
    if(items.length > 0){
      ...
      fragment.find(".pay").append(`
      <p class="lead">Do zapłaty: ${tot}zł<a class="btn btn-success  float-right" href="#" role="button">Zapłać</a></p>
      <br><br>
    `);
    }
