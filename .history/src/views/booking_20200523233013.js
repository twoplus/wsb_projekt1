...
export const booking = () => {
  ...
  return servePaths.reDirect("rooms").then(data => { 
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row mx-5 alert d-flex justify-content-between alert myro alert-secondary">${bookedRoom.name} ${bookedRoom.price}zł - 
      ${myOrderFin.start.day} ${myOrderFin.start.month} ${myOrderFin.start.year} -> ${myOrderFin.end.day} ${myOrderFin.end.month} ${myOrderFin.start.year}
  ...
  //return servePaths.reDirect("rooms").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      //${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}
      // const romm = () => {
      //   const frag = fragment;
      //   return servePaths.reDirect("rooms").then(data => {
      //     // const serach = (roo) => {
      ...
      //     // }
      //     // const bookedRoom = data.filter(serach)[0]
      //     // //outroom = bookedRoom.name
      //     frag.append("dupa dsdsdsdsdsds");
      //     return "chuj kuraw mac";
      //   });
      // }
      ...
      //console.log(romm())
      fragment.find(".addhere").append(readRoom);
