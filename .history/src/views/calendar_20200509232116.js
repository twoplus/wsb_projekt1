...
export class Calendar  {
    constructor(){
        ...
        this.getM = (x) => {
            ...
            //const out = this.months[ x<0 ? x%12+12 : x%12 ];
            //if x+
            const rev = [...this.months];//.reverse();
            rev.reverse();
            console.log(x);
            const out = x > 0 ? this.months[x%12] : rev[(Math.abx-1)%12];
            console.log(out);
            //console.log("--",x<0 ? x%12+11 : x%12);
            return out;
        }
        this.getY = (x,y) => {
            //console.log("::",x);
            //x = x < 0 ? parseInt(x+12) - 1 : parseInt(x/12);
            //console.log("=>",x,x/12 ,y);
            return y+x;
        }
