...
export const booking = () => {
  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    if(!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row m-5 alert alert-dark">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}</div>`)
