...
export const booking = () => {
  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row m-5 alert alert-dark">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}
      <a class="btn btn-danger btn-del float-right" href="#" role="button"><i class="fa fa-trash" aria-hidden="true"></i></a>
      </div>`)
      fragment.find(".addhere").append(`<div class="row m-5 alert alert-dark">${myOrderFin.treatments} </div>`)
