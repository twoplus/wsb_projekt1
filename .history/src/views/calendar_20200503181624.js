...
export const calendar = (x) => {
    ...
    const nextMonth = months[parseInt(selectedMonth) + 1 > 11 ? (parseInt(selectedMonth) + 1) -10 : parseInt(selectedMonth) + 1];
    ...
    //    });
      ...
      [...Array(getAllDays(2020,selectedMonth)).keys()].forEach(element => {
         fragment.find(".set-one").append(`<li>${element+1}</li>`); 
      });
    ...
    //      });
        ...
        [...Array(getAllDays(2020,selectedMonth+1)).keys()].forEach(element => {
           fragment.find(".set-two").append(`<li>${element+1}</li>`); 
        });
