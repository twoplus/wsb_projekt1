...
export const booking = () => {
  ...
  //return servePaths.reDirect("rooms").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row mx-5 alert d-flex justify-content-between alert alert-secondary">zupa
      </div>`)
      
     //${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}
     
       let allrooms = [];
       servePaths.reDirect("rooms").then(data => {
          const serach = (roo) => {
              return roo.id == 2? roo: false;
          }
          const bookedRoom = data.filter(serach)
          console.log(bookedRoom[0].name)
       });
