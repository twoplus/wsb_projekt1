...
export const booking = () => {
  ...
  return servePaths.reDirect("rooms").then(data => { 
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row mx-5 alert d-flex justify-content-between alert myro alert-secondary">${bookedRoom.name}  - 
      ${myOrderFin.start.day} ${myOrderFin.start.month} ${myOrderFin.start.year} -> ${myOrderFin.end.day} ${myOrderFin.end.month} ${myOrderFin.end.year} ${bookedRoom.price}zł
