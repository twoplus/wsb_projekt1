...
export const booking = () => {
  ...
  return servePaths.reDirect("booking").then(items => {
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      fragment.find(".addhere").append(`<div class="row m-5 alert d-flex justify-content-between alert alert-secondary">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}
      <a class="btn btn-danger btn-del" href="#" role="button"><i class="fa fa-trash" aria-hidden="true"></i></a>
      </div>`)
      myOrderFin.treatments.forEach(
        (id) => {
          fragment.find(".addhere").append(`<div class="row m-5 alert d-flex justify-content-between alert alert-secondary">${id} 
          <a class="btn btn-danger btn-del" href="#" role="button"><i class="fa fa-trash" aria-hidden="true"></i></a>
          </div>`)
        }
      )
      fragment.find(".addhere").append(`<div class="row m-5 alert d-flex justify-content-between alert alert-info">${myOrderFin.room} ${myOrderFin.start.day} ${myOrderFin.start.month} - ${myOrderFin.end.day} ${myOrderFin.end.month}
      <a class="btn btn-primary btn-add float-right" href="#" role="button"><i class="fa fa-plus" aria-hidden="true"></i></a>
      </div>`)
