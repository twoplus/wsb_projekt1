...
export const booking = () => {
  ...
  return servePaths.reDirect("rooms").then(data => { 
    ...
    if(!!!myOrderFin){//items.length != 0
      ...
      Wybierz zabiegi od nowa
      <a class="btn btn-primary btn-add text-white"  role="button"><i class="fa fa-plus" aria-hidden="true"></i></a>
      </div>`)
      fragment.find(".addhere").append(`<div class="row mx-5 alert d-flex justify-content-between ">
      Razem do zapłaty ${total}zł
      <a class="btn btn-success btn-pay text-white"  role="button">Przejdź do kasy</a>
      </div>`)
      //Remove  Treatments
      fragment.find(".btn-del").on("click",(event) => {
        delete myOrderFin.treatments;
      ...
      fragment.find(".btn-del-r").on("click",(event) => {
        ...
        delete myOrderFin.start;
        delete myOrderFin.end;
        localStore.write(myOrderFin);
        location.pathname = "/booking";
      })
      //Add new treatments 
      fragment.find(".btn-add").on("click",(event) => {
        location.pathname = "/treatments";
      })
      //Checkout
      fragment.find(".btn-pay").on("click",(event) => {
        if(!$.isEmptyObject(myOrderFin)){
          alert("chwilowo nie przujmujemy płatności online. Rezerwacja została przyjęta. Płatności dokonasz po przyjedzie w bilonie.");
        } else {
          alert("Ty, a może zamów coś najpierw!")
        }
